...
public class ExcelConnector {
	...
	public void deleteBookFromExcel(Book temp, String path) throws IOException {
	...
	public void updateExcel(String path) {
		...
		try {
			for (int i = 0; i < books.size(); i++) {
				deleteBookFromExcel(books.get(i), path);
	...
	public XSSFCellStyle changeCellStyle(XSSFWorkbook workbook,int num){
		...
		if(num < 5){
			XSSFColor color = new XSSFColor(new Color(153,0,0));		//heavy red
			...
			XSSFColor color = new XSSFColor(new Color(0,102,0));		//heavy green
			newCellStyle.setFillForegroundColor(color);
			newCellStyle.setFillPattern(CellStyle.SOLID_FOREGROUND);
		}
